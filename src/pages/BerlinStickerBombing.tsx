...
const campaignImage = '/sticker.png'; // Use berlin-final.png for visual consistency

const BerlinStickerBombing: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    const { data, error } = await supabase
      .from("berlin_answers")
      .insert([{ campaign: "sticker-bombing", answers: input }])
      .select();
    setIsLoading(false);
    if (!error && data) {
      setInput("");
  ...
  return (
    <div className="berlin-campaign-page max-w-full mx-auto px-0" style={{ paddingTop: 0, paddingBottom: 0 }}>
      ...
      <div className="w-full" style={{ marginBottom: '16px', marginTop: 0 }}>
        <img src={campaignImage} alt="Sticker Bombing Ad" className="berlin-banner-img" />
      </div>
      <div className="max-w-2xl mx-auto px-4">
        {/* <h1 className="text-3xl font-bold mb-4 text-center">Sticker Bombing Kampagne</h1> */}
...
export default BerlinStickerBombing; 